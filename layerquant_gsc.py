...
class SpeechCommandsDataset(Dataset):
    def __init__(
        ...
        self.mfcc_transform = torchaudio.transforms.MFCC(
            ...
            melkwargs={
                ...
                "n_mels": max(n_mfcc, 32),
                "center": True,
                "power": 2.0,
            },
            log_mels=True,
        )
        self.labels = [label_to_index[self.base_dataset.get_metadata(i)[2]] for i in range(len(self.base_dataset))]

    def __len__(self):
        return len(self.base_dataset)

    def _fix_waveform_length(self, waveform):
        target_num_samples = self.sample_rate
    ...
    def _fix_time_frames(self, feature):
        current_frames = feature.size(-1)
        if current_frames < self.max_frames:
            padding = self.max_frames - current_frames
    ...
    def __getitem__(self, index):
        waveform, sample_rate, label, _, _ = self.base_dataset[index]
        ...
        mfcc = self._fix_time_frames(mfcc)
...
def parse_args():
    ...
    parser.add_argument("--data-root", type=str, default="/autodl-tmp/gsc")
    ...
    parser.add_argument("--log-prefix", type=str, default="layerquant_gsc")
    parser.add_argument("--sample-rate", type=int, default=16000)
    parser.add_argument("--n-mfcc", type=int, default=32)
    parser.add_argument("--n-fft", type=int, default=1024)
    parser.add_argument("--hop-length", type=int, default=160)
    parser.add_argument("--win-length", type=int, default=400)
    parser.add_argument("--max-frames", type=int, default=32)
    parser.add_argument("--gsc-version", type=str, default="speech_commands_v0.01")
    parser.add_argument(
        "--download-gsc",
        dest="download_gsc",
        action="store_true",
        help="Download Speech Commands with torchaudio if it is not already present.",
    )
    parser.add_argument(
        "--no-download-gsc",
        dest="download_gsc",
        action="store_false",
        help="Do not download Speech Commands automatically.",
    )
    parser.add_argument(
        "--enable-gdc",
        dest="enable_gdc",
    ...
    parser.set_defaults(enable_gdc=True, download_gsc=True)
...
def build_resnet18(num_classes, in_channels=1):
...
def build_label_to_index(*datasets):
    labels = set()
    for dataset in datasets:
        for index in range(len(dataset)):
            _, _, label, _, _ = dataset.get_metadata(index)
            labels.add(label)
    label_list = sorted(labels)
    return {label: idx for idx, label in enumerate(label_list)}


def load_datasets(data_root, args):
    os.makedirs(data_root, exist_ok=True)
    common_kwargs = {
        "root": data_root,
        "url": args.gsc_version,
        "download": args.download_gsc,
    }
    train_base = SPEECHCOMMANDS(subset="training", **common_kwargs)
    validation_base = SPEECHCOMMANDS(subset="validation", **common_kwargs)
    test_base = SPEECHCOMMANDS(subset="testing", **common_kwargs)

    label_to_index = build_label_to_index(train_base, validation_base, test_base)
    dataset_kwargs = {
    ...
    train_set = SpeechCommandsDataset(train_base, **dataset_kwargs)
    validation_set = SpeechCommandsDataset(validation_base, **dataset_kwargs)
    test_set = SpeechCommandsDataset(test_base, **dataset_kwargs)
    return train_set, validation_set, test_set, label_to_index
...
def main():
    ...
    train_set, validation_set, test_set, label_to_index = load_datasets(args.data_root, args)
    ...
    global_model = build_resnet18(num_classes=num_classes, in_channels=1).to(device)
    ...
    logger.info("--- Speech Commands Root: {} ---".format(args.data_root))
    logger.info("--- Num Classes: {} ---".format(num_classes))
    logger.info("--- Train/Val/Test Sizes: {}/{}/{} ---".format(
    ...
    for round_idx in range(1, args.global_rounds + 1):
        ...
        for client_id in selected_clients:
            client_model = build_resnet18(num_classes=num_classes, in_channels=1).to(device)
...
# python3 /Users/chentonghao/WASA/layerquant_gsc.py \
#   --data-root /autodl-tmp/gsc \
#   --num-clients 10 \
#   --clients-per-round 5 \
...
#   --n-mfcc 32 \
#   --max-frames 32 \
#   --gsc-version speech_commands_v0.01 \
#   --download-gsc \
#   --quant-min-bit 2 \
#   --quant-max-bit 8 \
#   --omega-scale 1.0 \
